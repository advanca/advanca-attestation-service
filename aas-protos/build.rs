...
fn main() {
    ...
    for (dir, package) in modules {
        ...
        for str_path in files {
            ...
            let mut filename = proto_path.file_stem().unwrap().to_os_string();
            filename.push(".rs");
            let wrapper_path = output_path.join(&filename);
            add_prelude(&wrapper_path);
        }
    }
}
...
fn add_prelude(filepath: &Path) {
    ...
    for line in &content_split {
        if line.starts_with("use protobuf::Message") {
            new_content.push("#[cfg(feature = \"sgx_enclave\")]".into());
